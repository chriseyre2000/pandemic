defmodule PandemicModel.Player do
  ...
  def new(role, city \\ :altanta, cards) do
  ...
  def direct_flight(%__MODULE__{} = player, %PlayerCard{:city => destination}, board) do
    {:ok, %{player | city: destination}, board}
  end
  
  def direct_flight(%__MODULE__{} = _player, %PlayerCard{:type => type} = _card, _board) do
    {:error, "That's a #{type} card, you need a city card for a direct flight."}
  end
  
  # def charter_flight(%__MODULE__{} = player, %PlayerCard{"type" => :city, "city" => city} = card, destination_city, board) do
  ...
  # def charter_flight(%__MODULE__{} = player, %PlayerCard{:type => type} = card, _board) do
  #   {:error, "That's a #{type} card, you need a city card for a charter flight."}
  # end

  def shuttle_flight(%__MODULE__{} = player, destination, %Board{} = board) do
    cond do
      not Board.research_station?(board, player.city) -> {:error, "You are not in a research station"}
  ...
  def build_a_research_station(%__MODULE__{} = player, %Board{} = board) do
    cond do
      ...
      true -> {:ok, player, %{ board: Board.add_research_station(board, player.city)}, board}
    end  
  end
  ...
  #     player.city != city -> {:error, "You need to be in the same city as the card to share knowlegde"}
  #     # card in player.cards -> %{:ok, 
  #     #                           %{player | cards: player.cards -- [card]}, 
  #     #                           %{ other_player | cards: other_players.cards ++ [card] }, 
  #     #                           board}
  #     #card in other_player.cards -> %{:ok, %{player | cards: player.cards ++ [card] }, %{ other_player | cards: other_players.cards -- [card] }, board}
  ...
  def cure_disease(%__MODULE__{} =player, cards, %Board{} = board) do
    cond do
      Enum.count(cards) != 5 -> {:error, "You need 5 cards and only supplied #{Enum.count(cards)}"}
      not cards |> Enum.all?(&(&1.type == :city)) -> {:error, "All the cards need to be city cards"}
      cards -- player.cards != [] -> {:error, "You don't have those cards"}
      cards |> Enum.map(&Cities.city_colour/1) |> Enum.frequencies() |> Map.values != [5] -> {:error, "You don't have 5 cards of the same colour"}
